...
global m_glv
    ...
    eth = m_glv.eth;  % allocate memory ?!
    eth.pos = pos;  eth.vn = vn;
    eth.sl = sin(pos(1));  eth.cl = cos(pos(1));  eth.tl = eth.sl/eth.cl; 
    eth.sl2 = eth.sl*eth.sl;  sl4 = eth.sl2*eth.sl2;
    sq = 1-m_glv.e2*eth.sl2;  sq2 = sqrt(sq);
    eth.RMh = m_glv.Re*(1-m_glv.e2)/sq/sq2+pos(3);
    eth.RNh = m_glv.Re/sq2+pos(3);  eth.clRNh = eth.cl*eth.RNh;
    eth.wnie = [0; m_glv.wie*eth.cl; m_glv.wie*eth.sl];
    vE_RNh = vn(1)/eth.RNh;
    eth.wnen = [-vn(2)/eth.RMh; vE_RNh; vE_RNh*eth.tl];
    eth.wnin = eth.wnie + eth.wnen;
    eth.wnien = eth.wnie + eth.wnin;
    eth.g = m_glv.g0*(1+5.27094e-3*eth.sl2+2.32718e-5*sl4)-3.086e-6*pos(3); % grs80
    eth.gn = [0;0;-eth.g];
